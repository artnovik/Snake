...
public class PickupSpawner : MonoBehaviour
{
    ...
    private IEnumerator CreateFruitRandom(bool playEatFruit = true)
    {
        while (true)
        {
            ...
            switch (GameManager.Instance.gateState)
            {
                case GateState.Closed:
                    ranX = Random.Range((int)WorldBorders.LeftBorder, (int)WorldBorders.CenterBorderLeft);
                    ranY = Random.Range((int)WorldBorders.BottomBorder, (int)WorldBorders.TopBorder);
                    break;
                case GateState.Opened:
                    ranX = Random.Range((int)WorldBorders.CenterBorderRight, (int)WorldBorders.RightBorder);
                    ranY = Random.Range((int)WorldBorders.BottomBorder, (int)WorldBorders.TopBorder);
                    break;
            }
            newPosition = new Vector3(ranX, ranY, 5.72f);
