...
public class ScoreManager : MonoBehaviour
{
    ...
    [SerializeField]
    private int openGateScore;

    [SerializeField]
    private int openExitScore;
    ...
    private void OnFruitAte(int scoreAddition, Vector3 position)
    {
        ...
        if (currentScore == openGateScore)
        {
            GameManager.Instance.gateState = GateState.Opened;
            gateClosed.SetActive(false);
            gateOpened.SetActive(true);
        }

        if (currentScore == openExitScore)
        {
            GameManager.Instance.gateState = GateState.ExitOpened;
